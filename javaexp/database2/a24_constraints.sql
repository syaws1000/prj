...
SELECT * FROM USER_CONSTRAINTS WHERE TABLE_NAME='STUDENT_COURSES';
INSERT INTO STUDENT_COURSES VALUES('1001','8001', SYSDATE);
INSERT INTO STUDENT_COURSES VALUES('1001','8002', SYSDATE);
INSERT INTO STUDENT_COURSES VALUES('1001','8003', SYSDATE);
INSERT INTO STUDENT_COURSES VALUES('1001','8001', SYSDATE); 
-- 두개의 컬럼 모두 다 동일한 것은 제약조건에 걸림..
-- CHECK 제약 조건 : 조약조건형식은 WHERE 조건문안에 선언형식과동일
CREATE TABLE STUDENT10(
	NAME VARCHAR2(50),
	GENDER CHAR(1) CONSTRAINT STUDENT10_GENDER_CK CHECK( GENDER IN('F','M') )
);
-- CHECK( 컬럼 IN( 데이터1, 데이터2) )  : CHECK 안에 WHERE 조건문 형식으로 
--  GENDER가 'F' 또는 'M' 일 때
INSERT INTO STUDENT10 VALUES('홍길동','M');
INSERT INTO STUDENT10 VALUES('홍길순','f'); -- 'M'/'F' 문자만 입력이 가능..
SELECT * FROM STUDENT10;
SELECT *
FROM EMP
WHERE SAL BETWEEN 1000 AND 3000;
-- EX) STUDENT11 테이블에 NAME과, GRADE를 만들고, 해당 GRADE 1~4 숫자만 입력가능하게 처리..
CREATE TABLE STUDENT11(
	NAME VARCHAR2(50),
	GRADE NUMBER CONSTRAINT STUDENT11_GRADE_CK CHECK( GRADE BETWEEN 1 AND 4 )
);
INSERT INTO STUDENT11 VALUES('홍길동', 3);
SELECT * FROM STUDENT11;
-- 외래키 제약조건,  특정한 컬럼에 데이터 입력/수정의 범위가 다른 테이블의 데이터를 기준으로 처리할 때
-- 주로 사용된다. 
-- 기본 형식   컬럼명 데이터유형  CONSTRAINT 제약조건명 REFERENCES 참조할테이블명(컬럼명)
-- EMP테이블에서 DEPTNO를 입력할 때, DEPT테이블의 DEPTNO를 참조해서 입력이 가능하다.
SELECT * FROM EMP11;
SELECT * FROM DEPT01;
CREATE TABLE EMP11(
	EMPNO NUMBER(4,0),
...
INSERT INTO EMP11 VALUES(1000,'홍길동',10);
INSERT INTO EMP11 VALUES(1001,'김길동',20);
INSERT INTO EMP11 VALUES(1002,'신길동',30);
INSERT INTO EMP11 VALUES(1003,'마길동',40);
INSERT INTO EMP11 VALUES(1004,'호길동',50); -- 참조 에러 발생
SELECT * FROM EMP11;
...
-- 테이블로 STUDENT12_SUBJECT 만들고, SID를 참조키로 설정하여 과목명(SUBJECT)과 학점(GRADE)을 표현하세요..
-- STUDENT12의 입력된 데이터에 따라서, STUDENT12_SUBJECT에거 SID를 입력할 수 있게 처리하세요.
...
INSERT INTO STUDENT12_SUBJECT VALUES('알고리즘개론',3.0, 1002); 
--STUDENT12테이블에 등록된 SID로만 수강신청 데이터를 처리가 가능하다.

SELECT * FROM STUDENT12_SUBJECT;
SELECT * FROM DEPT;
SELECT * FROM EMP11;
-- DEPTNO NUMBER(2,0)  REFERENCES DEPT(DEPTNO)
INSERT INTO EMP11 VALUES(1005,'하길동',50);  -- 부서정보가 50번이 없기에 에러발생..
