...
-- PRIMARY KEY : 반드시 입력을 하면서 데이터로 식별을 할 때, 사용한다.
CREATE TABLE STUDENT06(
	SNO NUMBER(5,0) CONSTRAINT STUDENT06_SNO_PK PRIMARY KEY
);
SELECT * FROM USER_CONSTRAINTS WHERE TABLE_NAME='STUDENT06';
INSERT INTO STUDENT06 VALUES(3);
SELECT * FROM STUDENT06;
-- EX) EMPLOYEES01 테이블에 EMPNO(PRIMARY KEY), ENAME(NOT NULL)을 설정하여 데이터를 입력하세요..
CREATE TABLE EMPLOYEES01(
	EMPNO NUMBER(4,0) CONSTRAINT EMPLOYEES01_EMPNO_PK PRIMARY KEY,
	ENAME VARCHAR2(50) NOT NULL
);
INSERT INTO EMPLOYEES01 VALUES(1001,'홍길동');
SELECT * FROM EMPLOYEES01;
SELECT * FROM USER_CONSTRAINTS WHERE TABLE_NAME='EMPLOYEES01';

-- 두개의 컬럼을 기준으로 PRIMARY KEY를 설정하는 경우..
-- 예를들어 쇼핑몰에서 가지고 있는 물건 정보와  구매하는 회원정보를 매칭할 때,
-- 구매테이블이라는 테이블이 만들어 지는데 이 때, 물건정보의 고유 PRIMARY 키를 가진 PROD_ID와
-- 회원테이블에서 회원의 고유 PRIMARY 키를 가진 MEMBER_ID가 구매테이블에 들어 가면 두개 컬럼으로 PRIMARY KEY로
-- 구매테이블의 식별한다. 이 때, 하나의 MEMBER가 다른 여러개의 물건 PROD_ID를 설정할 수 있어 두개의 컬럼을 기준으로
-- PRIMARY KEY를 지정한다.
...
SELECT * FROM USER_CONSTRAINTS WHERE TABLE_NAME='ORDER_DETAILS';
INSERT INTO ORDER_DETAILS VALUES(101,1,10,50.0);
INSERT INTO ORDER_DETAILS VALUES(101,2,10,50.0);
INSERT INTO ORDER_DETAILS VALUES(102,1,10,50.0);
SELECT * FROM ORDER_DETAILS;

-- EX) STUDENT_COURSES 테이블로 STUDENT_ID, COURSE_ID, ENROLLMENT_DATE 로
--     학생의 수강 신청을 학생의 아이디와 과목의 아이디 두개의 PRIMARY KEY로 설정해서 처리하고,
--     데이터를 입력하세요...
